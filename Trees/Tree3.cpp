...
void levelOrder_02(Node *node)
{
    ...
    que.push(node);
    que.push(nullptr);
    while (que.size() != 1) // stop the loop when u have the only nullptr in your queue
    {
        Node *rvtx = que.front();
        que.pop();
        cout << rvtx->data << " ";
...
void verticalOrderTraversal(Node *node)
{
    ...
    width(node, 0, maxMin);
    int w = maxMin.second - maxMin.first + 1; // size of tree's entire width
    vector<vector<int>> ans(w, vector<int>()); // array to store the vertical order traversal

    queue<pair<Node *, int>> que; // node and level pair
    que.push({node, -maxMin.first}); // -maxMin.first = level
    ...
    while (que.size() != 0)
    {
        ...
        while (size-- > 0)
        {
            ...
            ans[rpair.second].push_back(rpair.first->data); // store the result int ans array
